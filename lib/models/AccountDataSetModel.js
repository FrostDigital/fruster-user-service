...
class AccountDataSetModel {
    ...
    async toViewModel(roleManager) {
        log.debug("Converts data model to view model for data", this.id);
        ...
        try {
            ...
            delete data.hashDate;

            // @ts-ignore
            delete data._id;

            if (data.roles)
                data.scopes = await roleManager.getScopesForRoles(data.roles);

            if (config.lowerCaseName) {
                if (data.firstName)
                    data.firstName = Utils.toTitleCase(data.firstName);
                if (data.lastName)
                    data.lastName = Utils.toTitleCase(data.lastName);
                if (data.middleName)
                    data.middleName = data.middleName ? Utils.toTitleCase(data.middleName) : undefined;
            }
