...
class CreateUserHandler {
    ...
    async handle(req) {
        try {
            ...
            this._passwordService.hashPassword(user);
            ...
            return {
                status: 201,
                data: createdUser.toViewModel(this._roleService)
            };
        } catch (err) {
            log.error(err);
            throw err;
    ...
    async  _validateInputData(data) {
        ...
        if (!utils.validateEmail(data.email))
            throw errors.invalidEmail(data.email);

        const invalidRoles = this._roleService.validateRoles(data.roles);
        ...
        if (!emailIsUnique)
            throw errors.emailNotUnique(data.email);
    }
