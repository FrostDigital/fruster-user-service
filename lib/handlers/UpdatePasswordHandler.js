...
class UpdatePasswordHandler {
    ...
    async handle(req) {
        ...
        user = await this._passwordService.hashPassword(user, newPassword);
    ...
    async _validatePassword(reqId, user, oldPassword) {
        if (!(await this._passwordService.validatePassword(user.password, user.salt, user.id, oldPassword, user.hashDate)))
