...
class ValidatePasswordHandler {
	 ...
	 */
	async handle({ data: { username, password, additionalQuery } }) {
		try {
			...
			const allUsers = await this._repo.getUsersByQueryInternal(query);
			// go for exact match if multiple users are found
			const user = (allUsers && allUsers.length > 1) ? allUsers.find(user => user.username === data.username) : [allUsers];
