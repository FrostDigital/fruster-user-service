...
class ValidatePasswordHandler {
    ...
    async handle(req) {
        try {
            ...
            const query = {};
            query[this._usernameValidationDbField] = { $regex: new RegExp(`^${username.replace(/([.*+?=^!:${}()|[\]\/\\])/g, '\\$1')}`, "i") };
            ...
            else if (user && this._validatePassword(user, password))
                return {
                    status: 200,
                    data: user.toViewModel(this._roleService)
                }
            else
    ...
    _validatePassword(user, password) {
        if (this._passwordService.validatePassword(user.password, user.salt, user.id, password, user.hashDate))
            return true;
        else
            throw deprecatedErrors.invalidUsernameOrPassword();
    }
