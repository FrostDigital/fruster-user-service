...
class ValidatePasswordHandler {
    ...
    async handle(req) {
        try {
            ...
            if (emailVerificationIsRequired && emailIsNotVerified)
    ...
    async _validatePassword(user, password) {
        if ((await this._passwordManager.validatePassword(user.password, user.salt, user.id, password, user.hashDate)))
            return true;
        else
            throw deprecatedErrors.invalidUsernameOrPassword();
    }
