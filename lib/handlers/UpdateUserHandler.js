...
class UpdateUserHandler {
    ...
    async handle(req) {
        ...
        // @ts-ignore
        [user, profile] = await this._profileManager.splitUserFields(updateUserData);

        const responseData = await this._updateInDatabase(userId, user);
        let returnUser = new UserModel(responseData);

        if (Object.keys(profile).length > 1) {
            //TODO: MAKE SURE THIS WORKS!
            const createdProfile = await this._profileManager.saveProfile(profile);
            returnUser = returnUser.concatWithProfile(createdProfile);
        }
