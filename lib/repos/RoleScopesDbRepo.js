...
class RoleScopesDbRepo extends AbstractRoleScopesRepo {
	 ...
	 */
	async getRoles() {
		return await this._collection.find({}).project({ _id: 0 }).toArray();
	}
	 ...
	 */
	async getRole(role) {
		return await this._collection.findOne({ role }, { projection: { _id: 0 } });
	}
	...
	async addRole(role, scopes = []) {
		...
		await this._collection.insertOne(new RoleModel(role, scopesToAdd));
	...
	async addRoles(roles) {
		await this._collection.insertMany(roles);
	}
