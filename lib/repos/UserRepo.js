...
class UserRepo {
	...
	async getUsersByQuery(query = {}, start = 0, limit = 0, filter = {}, sort = {}, expand = false) {
		...
		try {
			...
			if (expand)
				[usersFromDatabase, totalCount] = await this._aggregateResult(query, start, limit, dbFilter, sort);
			else
	 ...
	 * @param {Object=} sort
	 */
	async _aggregateResult(query, start = 0, limit = 0, filter, sort) {
		/** @type {Array} */
		const aggregation = getAggregationBeforeLimits();

		if (Object.keys(filter).length > 0)
			aggregation.push({ $project: filter });
		...
		if (Object.keys(sort).length > 0)
			aggregationQuery = aggregationQuery.sort(sort);
		...
		if (limit) {
			/** @type {Array} */
			const countAggregationQuery = getAggregationBeforeLimits()
		...
		/** @return {Array} */
		function getAggregationBeforeLimits() {
			return [
	...
	async deleteUser(userId) {
		try {
			const response = await this._collection.deleteOne({
				id: userId
			});
