...
class UserRepo {
	 ...
	 */
	async getUsersByQuery(query = {}, start = 0, limit = 0, filter = {}, sort = {}, expand = false) {
		...
		try {
			...
			// if (expand)
			[usersFromDatabase, totalCount] = await this._aggregateResult(query, start, limit, dbFilter, sort, expand);
			// else
	 ...
	 * @param {Object=} sort
	 */
	async _aggregateResult(query, start = 0, limit = 0, filter, sort, doExpand) { // TODO: Have flag for case insensitive sort?
		/** @type {Array} */
		...
		if (hasSort) {
			...
			Object.keys(sort).forEach(sortKey => {
				$project["caseInsensitiveSortVar" + sortKey] = { "$toLower": `$${sortKey}` };
				sortObj["caseInsensitiveSortVar" + sortKey] = sort[sortKey];
			});
			// TODO: should be possible to sort result with `sort` fails because it converts number to string
			//TODO: should get users by query fails: dunno why
		}
