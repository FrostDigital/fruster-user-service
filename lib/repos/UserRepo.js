...
class UserRepo {
	...
	async _aggregateResult(query, start = 0, limit = 0, filter, sort) {
		/** @type {Array} */
		const aggregation = this._getExpandAggregation(query);
		...
		const usersFromDatabase = await aggregationQuery.toArray();
		let totalCount;

		if (limit) {
			/** @type {Array} */
			const countAggregationQuery = this._getExpandAggregation(query)
