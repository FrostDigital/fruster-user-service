	...
	async _aggregateResult(query, start = 0, limit = 0, filter, sort, doExpand, caseInsensitiveSort) {
		...
		if (hasSort) {
			Object.keys(sort).forEach(sortKey => {
				...
				if (doExpand && !sortKey.includes("profile"))
					newSortKey = `user.${sortKey}`;
				...
				if (caseInsensitiveSort) {
					...
					sortObj["__caseInsensitiveSortVar" + newSortKey] = sort[sortKey];
				} else {
					sortObj[newSortKey] = sort[sortKey];
				}
			});
		}
		...
		let aggregationQuery = this._collection.aggregate(aggregation);
