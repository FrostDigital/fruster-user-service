...
class ProfileManager {
	 ...
	 */
    getProfilesByQuery(query, start, limit, filter, sort) {
        return this._profileRepo.getProfilesByQuery(query, start, limit, filter, sort);
    }
    ...
    async expandUsersWithProfiles(users, filter = {}) {
        const [profiles] = await this.getProfilesByQuery({ id: { $in: users.map(u => u.id) } }, undefined, undefined, filter);
        const profilesById = {};
