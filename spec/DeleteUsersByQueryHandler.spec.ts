...
import Publishes from "../lib/Publishes";
...
describe("DeleteUsersByQueryHandler", () => {
	...
	it(`should publish ${Publishes.subjects.USER_DELETED} for each removed user`, async () => {
		try {
			...
			const profilesBeforeDelete: any = await db.collection(constants.collections.PROFILES).find(reqData).toArray();
