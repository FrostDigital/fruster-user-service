...
import { createIndexes } from "../fruster-user-service";
...
describe("CreateUserHandler", () => {
	...
	it("should be possible to create user split into user and profile datasets", async () => {
		...
		const userFromDatabase: any = await db.collection(constants.collections.USERS).findOne({ id: response.data.id });
		const profileFromDatabase: any = await db.collection(constants.collections.PROFILES).findOne({ id: response.data.profile.id });
	...
	it("should validate password when creating user", async () => {
		...
		const err: any = await SpecUtils.busRequestExpectError(constants.endpoints.service.CREATE_USER, user);
	...
	it("should validate indexed duplicates email when creating user", async () => {
		...
		const user2: any = mocks.getUserObject();
		user2.email = "email2@email.com";

		const err: any = await SpecUtils.busRequestExpectError(constants.endpoints.service.CREATE_USER, user2);
	...
	it("should validate required fields when creating user", async () => {
		let user: any = mocks.getUserObject();
		...
		async function doRequest(userToCreate: any) {
			try {
	...
	it("should resend verification mail when updating email if conf.requireEmailVerification is set to true and conf.emailVerificationTemplateByRole and config.requirePasswordOnEmailUpdate set", async () => {
		...
		const testUser: any = await db.collection(constants.collections.USERS).findOne({ id: createdUser.id });
