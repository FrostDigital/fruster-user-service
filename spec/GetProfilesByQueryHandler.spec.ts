...
import TestUtils from "./support/SpecUtils";
...
describe("GetProfilesByQueryHandler", () => {
	...
	it("should be possible to get profiles by a simple query", async () => {
		...
		const { data } = await TestUtils.busRequest({
			...
			data: {
				query: {
					id: { $in: testUsers.map((u: any) => u.id).filter((u: any, i: number) => i % 2 !== 0) }
				},
		...
		const selectedUsers = testUsers.filter((u: any, i: number) => i % 2 !== 0);
	...
	it("should be possible to paginate result from get profiles by query with `limit`", async () => {
		...
		const { data } = await TestUtils.busRequest({
			subject: constants.endpoints.service.GET_PROFILES_BY_QUERY,
			data: {
				query: { id: { $in: testUsers.map((u: any) => u.id) } },
				limit: 3,
				sort: { firstName: 1 } //Otherwise sort by id, because of profile.id index
			}
		});
	...
	it("should be possible to sort result with `sort`", async () => {
		const testUsers = await createTestUsers(10);
		const { data: data1 } = await doRequest(1, testUsers.slice(0, testUsers.length - 1).map((u: any) => u.id));
		const { data: data2 } = await doRequest(-1, testUsers.slice(0, testUsers.length - 1).map((u: any) => u.id));
		...
		async function doRequest(sort: number, ids: string[]) {
	...
	it("should be possible to sort result with case insensitive`sort`", async () => {
		...
		const { data: { profiles: profilesResp1, totalCount: totalCountResp1 } } = await doRequest(1, testUsers.map((u: any) => u.id));
		const { data: { profiles: profilesResp2, totalCount: totalCountResp2 } } = await doRequest(-1, testUsers.map((u: any) => u.id));
	...
	async function createTestUsers(number = 2, startAt = 0) {
		...
		function getTestUserData(username: string, i: number) {
			return {
